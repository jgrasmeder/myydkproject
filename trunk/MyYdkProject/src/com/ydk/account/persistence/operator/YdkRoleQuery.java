 ...
 *
 */
public class YdkRoleQuery extends QueryOperator<YdkRole> {
	
	public YdkRoleQuery(EntityManager em)
	{
		setEntityManager(em);
		validate();
		setMaxResults(YdkRoleQuery.MAX_RESULTS_NUM);
	...
	public String getEntityClassName() {
		...
		return YdkRole.class.getSimpleName();
	}
	...
	public String getWholeEntityClassName() {
		...
		return "com.ydk.account.persistence.entity.YdkRole";
	}
	
	/*
	 * Search User with specified field
	 */
	public List<YdkRole> searchYdkRoleByField(String name, Object value)
	{
		setQueryAttribute(CASE_INSENSITIVE_RESTRICTION_SEARCH_ALL);
		addRestrictionParameter(name, value);
		List<YdkRole> list = getResultList();
		return list;
	}
	
	/*
	 * Search YdkRole with specified field
	 */
	public YdkRole searchYdkRoleByName(String name)
	{
		setQueryAttribute(CASE_SENSITIVE_RESTRICTION_SEARCH);
		addRestrictionParameter("roleName", name);
		List<YdkRole> list = getResultList();
