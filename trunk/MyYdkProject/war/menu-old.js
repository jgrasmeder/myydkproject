...
function activeMenuItem  (menuItem){
		if (curMenuItem ){
			curMenuItem.className = "menuitem";
		}
		curMenuItem = menuItem;
		curMenuItem.className = "menuitem-active";
}
...
var menuManager = {
	...
	closefolder : "images/sub_menu.gif", //set image path to "closed" folder image
	openfolder : "images/sub_menu_coll.gif", //set image path to "open" folder image
	curMenu : null,
	curSubMenu : new Array(),
	createMenuById : function (pId, mId, hidden){
		var p = document.getElementById(pId);
		this.createMenu(p, mId, hidden);
	},
	createMenu : function(parentNode, menuId, hidden){
	...
	toggleSubMenuBox : function (subMenuBar, rel){
...
/*		subMenuBar.parentNode.style.border = outline;
	...
	foldSubMenu : function (subMenuBar){
		var display = "none";
		var outline = "";
		subMenuBar.setAttribute("rel", "closed"); //let's close it
		subMenuBar.style.backgroundImage="url(" + menuManager.openfolder + ")";
	...
	unfoldSubMenu : function (subMenuBar){
		var display = "block";
	...
	onChangeMenu : function (obj){
		var id = obj.getAttribute("menu");
		this.switchMenu(id);
	},
	
	rememberstate : function(menuGroupId, durationdays){ //store index of opened ULs relative to other ULs in Tree into cookie
	...
	hookEvent : function(target, functionref, tasktype){ //assign a function to execute to an event handler (ie: onunload)
