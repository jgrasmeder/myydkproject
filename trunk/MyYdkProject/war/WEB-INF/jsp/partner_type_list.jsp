...
<table class="table2"  width="100%" border="0" cellspacing="0" >
  ...
  <c:choose>
	  ...
	  <td>
	  	...
	  	<a href="#" onClick="document.pageDataForm.action='<c:url value="/modifyPartnerType.do"/>';
	...
	<c:otherwise>
	  ...
	  <td>${item.id}</td>
	  <td>${item.type}</td>
	  <td><a href="<c:url value="/listPartnerType.do?itemId=${item.id}&pageNumber=${pageDataList.page}"/>">修改</a></td>
  	</c:otherwise>
  </c:choose>
  </tr>
  </c:forEach>
