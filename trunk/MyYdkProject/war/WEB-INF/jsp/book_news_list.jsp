...
<table class="table2"  width="100%" border="0" cellspacing="0" >
  ...
  <c:choose>
	  ...
	  <td>
	  	<form:select path="instance.bookNewsCategory.id" cssStyle="width:100px">
    		<c:forEach var="type" items="${typeList}">
    			<option value="${type.id}" <c:if test="${type.id == dataForm.instance.bookNewsCategory.id}">selected</c:if>>${type.name}</option>
    		</c:forEach>
    	</form:select>
      </td>
	  ...
	  <td>
	  	...
	  	<a href="#" onClick="document.pageDataForm.action='<c:url value="/modifyBookNews.do"/>';
		  document.pageDataForm.submit();return true;">保存</a></td>
	</c:when>
	<c:otherwise>
	  <td><form:checkbox path="keys" value="${item.id}"/></td>
	  <td><a href="<c:url value="/viewBookNews.do?itemId=${item.id}"/>">${item.title}</a></td>
	  <td>${item.bookNewsCategory.name}</td>
	  <td>${item.date}</td>
	  ...
	  <td><a href="<c:url value="/listBookNews.do?itemId=${item.id}&pageNumber=${pageDataList.page}"/>">修改</a></td>
  	</c:otherwise>
  </c:choose>
  </tr>
  </c:forEach>
